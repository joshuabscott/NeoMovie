...
namespace NeoMovie.Services
{
    public class TMDBMovieService : IRemoteMovieService
    {
        ...
        public async Task<ActorDetail> ActorDetailAsync(int id)
        {
            ...
            {
                { "api_key", _appSettings.NeoMovieSettings.TmDbApiKey },
        ...
        public async Task<MovieSearch> SearchMoviesAsync(MovieCategory category, int count)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                movieSearch.results.ToList().ForEach(r => r.poster_path = $"{_appSettings.TMDBSettings.BaseImagePath}/{_appSettings.NeoMovieSettings.DefaultPosterSize}/{r.poster_path}");
            }
            return movieSearch;
        }
    }
